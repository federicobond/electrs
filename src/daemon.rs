...
impl Connection {
    fn new(addr: &String, cookie_getter: Arc<CookieGetter>, signal: Waiter) -> Result<Connection> {
        let resolved_addr = addr
            .to_owned()
            .to_socket_addrs()
            .map_err(|_| ErrorKind::Connection("failed to resolve addr".to_owned()))
            .and_then(|mut iter| {
                iter.find(|addr| addr.is_ipv4())
                    .ok_or(ErrorKind::Connection("failed to resolve addr".to_owned()))
            })?;
